...
def getRestaurant():
    ...
    # handles closed restaurant constraint
    for restaurant in restaurants:
        if restaurant['is_closed']:
            final[restaurant['name']] += 1

    # gets constraints from everyone
    ...
    # counts number of contraints for each restaurant
    start_time = time.time()
    for restaurant in restaurants:
        if restaurant['distance'] > distance_constraint:
            final[restaurant['name']] += 1
        if restaurant['rating'] < rating_constraint:
            final[restaurant['name']] += 1
        if 'price' in restaurant.keys():
            if len(restaurant['price']) > price_constraint:
                final[restaurant['name']] += 1
        for category in restaurant["categories"]:
    ...
    # returns least constrained restaurant
    sorted_d = sorted((value, key) for (key,value) in final.items())
    print(sorted_d[0][1])
